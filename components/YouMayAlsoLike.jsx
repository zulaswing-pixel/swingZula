    ...
    return (
      <div className="flex items-center gap-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            key={star}
            className={`w-3 h-3 ${star <= Math.round(displayRating) ? "fill-yellow-400 text-yellow-400" : "fill-gray-200 text-gray-200"
              }`}
          />
        ))}
      </div>
        ...
        <div className="p-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              disabled={loading}
              className={`border-2 border-blue-600 text-blue-600 px-3 py-2 rounded-lg text-xs font-medium hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center gap-1.5 cursor-pointer  ${loading ? "opacity-50 cursor-not-allowed" : ""
                }`}
            >
...
export default function YouMayAlsoLikeSlider({ products }) {
  ...
  useEffect(() => {
    ...
    if (products?.length > 0 && mounted) {
      ...
      console.log("First product featuredImage:", products[0]?.featuredImage);
      console.log("Window width:", windowWidth);
      console.log("==================");
    }
  }, [products, mounted, windowWidth]);
  ...
  return (
    <>
      <section className="mt-12 relative bg-gradient-to-br from-sky-50 to-blue-50 py-8">
          ...
          <div className="relative px-4 md:px-12">
            <Slider {...settings} key={windowWidth}>
              {products.map((product, index) => (
                <div key={product.id || product.handle || index} className="px-2">
